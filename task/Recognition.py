...
def recognizeGloss(config):
    ...
    while True:
        try:
            ...
            sys.exit(0)
        video = np.frombuffer(serialized_video, dtype=np.uint8).reshape(config['num_frames'],3, config['crop_height'], config['crop_width'])
